...
impl ::protobuf::Message for Person {
    ...
    fn parse_from(is: &mut protobuf::CodedInputStream) -> protobuf::Result<Self> {
        let mut r: Self = protobuf::Message::new();
        r.merge_from(is)?;
        r.check_initialized()?;
        Ok(r)
    }
    ...
    fn write_length_delimited_to(&self, os: &mut protobuf::CodedOutputStream) -> protobuf::Result<()> {
        let size = self.compute_size();
        let size = check_message_size(size)?;
    ...
    fn write_length_delimited_to_vec(&self, vec: &mut Vec<u8>) -> protobuf::Result<()> {
        let mut os = protobuf::CodedOutputStream::vec(vec);
    ...
    fn parse_from_reader(reader: &mut dyn std::io::Read) -> protobuf::Result<Self> {
        let mut is = protobuf::CodedInputStream::new(reader);
        let r = protobuf::Message::parse_from(&mut is)?;
        is.check_eof()?;
        Ok(r)
    }
    ...
    fn check_initialized(&self) -> protobuf::Result<()> {
    ...
    fn write_to_writer(&self, w: &mut dyn std::io::Write) -> protobuf::Result<()> {
        w.with_coded_output_stream(|os| self.write_to(os))
    }

    fn write_to_vec(&self, v: &mut Vec<u8>) -> protobuf::Result<()> {
    ...
    fn write_length_delimited_to_writer(&self, w: &mut dyn std::io::Write) -> protobuf::Result<()> {
        w.with_coded_output_stream(|os| self.write_length_delimited_to(os))
    }

    fn write_length_delimited_to_bytes(&self) -> protobuf::Result<Vec<u8>> {
        let mut v = Vec::new();
